...
def getcomments(page=5, id="29004400"):
    assert page>=1 and page<=100
    with open("comment.txt", "w", encoding='utf-8') as f:
        for i in range(page):
            curr_time = int(time.time() * 1000)

            para_dir={"csrf_token": "", "cursor": "%s" % curr_time, "offset": str(i*20), "orderType": "2", "pageNo": str(i+1),
                                 "pageSize": "20", "rid": "R_SO_4_"+id, "threadId": "R_SO_4_"+id}

            if i>=10:
                para_dir["offset"]="0"

            param1 = json.dumps(para_dir)
            ...
            r = requests.post(url, headers=headers, data=param_data, )

            f.write("爬取第"+str(i+1)+"页"+"*"*100+'\n')
            print("爬取第"+str(i+1)+"页")
            for comment in json.loads(r.text)["data"]["comments"]:
                f.write(comment["content"]+'\n')

if __name__=="__main__":
    getcomments(page=100, id="536622304")
